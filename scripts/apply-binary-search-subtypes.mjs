// Merges data/binary-search-subtypes.json into data/problems.json's `techniques` field.
// Keeps the existing "Binary Search" label and adds the specific subtype as an additional entry.
//
// Run: node scripts/apply-binary-search-subtypes.mjs
...
const SUBTYPES_PATH = path.join(__dirname, '..', 'data', 'binary-search-subtypes.json')
...
async function main() {
  ...
  console.log(`Applied ${applied} binary search subtype labels.`)
}
